...
class TESModelTextureSwap : public TESModel
	...
	public:
		...
		AlternateTexture*	swaps;		// 28 - MODS
		UInt32				count;	// 30
		UInt32				pad34;					// 34
	};
...
public:
	...
	EnchantmentItem*			enchantment;			// 08 - EITM
	CastingType16				castingType;			// 10
	UInt16						maxCharge;	// 12 - EAMT
	UInt32						pad14;
};
...
public:
	...
	virtual void	RestoreActorValue(UInt32 a_modifier, UInt32 a_akValue, float a_value);
...
public:
	...
	struct Data
	{
		TESForm*				item;			// 00 - Can be ARMO or ARMA
